...
def p_normal_statement(p):
   '''expression_statement : statement_expression KW_semi'''
...
def p_if_then_statement(p):
   ...
   p[0] = Node("if_then_statement", [p[1], p[2],p[3],p[4],p[5]])
...
def p_if_then_else_statement(p):
   ...
   p[0] = Node("if_then_else_statement", [p[1], p[2],p[3],p[4],p[5],p[6],p[7]])
...
def p_if_then_else_statement_precedence(p):
   ...
   p[0] = Node("if_then_else_statement_precedence", [p[1], p[2],p[3],p[4],p[5],p[6],p[7]])
...
def p_while_statement(p):
   ...
   p[0] = Node("while_statement", [p[1], p[2],p[3],p[4],p[5]])
...
def p_for_statement(p):
   ...
   p[0] = Node("for_statement", [p[1], p[2],p[3],p[4],p[5]])


def p_for_logic(p):
   ''' for_logic : for_update | for_update KW_semi for_logic '''
...
def p_for_loop(p):
   ...
   p[0] = Node("for_loop", [p[1], p[2],p[3],p[4],p[5]])
...
def p_return_statement(p):
   '''return_statement : KW_return expression_optional KW_semi '''



   p[0] = Node("return_statement", [p[1],p[2],p[3]])
# CLASS DECLARATION
...
def p_class_header(p):
   ...
   p[0] = Node("class_header", [p[1], p[2],p[3],p[4],p[5]])
...
def p_class_param_clause(p):
   '''class_param_clause : KW_LPAREN class_params_opt KW_RPAREN'''



   p[0] = Node("class_param_clause", [p[1],p[2],p[3]])


def p_class_params_opt(p):
...
def p_class_params(p):
   ...
   if(len(p) == 2):
      ...
      p[0] = Node("class_params", [p[1], p[2],p[3]])
...
def p_class_template(p):
   ...
   p[0] = Node("class_template", [p[1], p[2],p[3],p[4],p[5]])
...
def p_method_header(p):
   ...
   p[0] = Node("method_header", [p[1],p[2]])
...
def p_fun_param_clause(p):
   ...
   p[0] = Node("fun_param_clause", [p[1],p[2],p[3]])
...
def p_fun_params(p):
   ...
   if(len(p) == 2):
      ...
      p[0] = Node("fun_params", [p[1], p[2],p[3]])
...
#write empty i have not written
