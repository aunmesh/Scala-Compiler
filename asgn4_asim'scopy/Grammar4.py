...
def p_dimension(p):
   ''' dimension : dimension TOK_lsqb expression TOK_rsqb | TOK_lsqb expression TOK_rsqb '''
...
def p_assignment_operator(p):
   ''' assignment_operator : TOK_assignment | TOK_mulassign | TOK_divassign | TOK_modassign | TOK_addassign | TOK_subassign '''

   leaf1 = create_children("LF_AssignOp", p[1])
   p[0] = Node("assignment_operator", [leaf1])

def p_conditional_or_expression(p):
   ''' conditional_or_expression : conditional_and_expression | conditional_or_expression TOK_or conditional_and_expression '''
...
def p_inclusive_or_expression(p):
   '''inclusive_or_expression : exclusive_or_expression  | inclusive_or_expression TOK_or_bitwise exclusive_or_expression'''
...
def p_exclusive_or_expression(p):
   ''' exclusive_or_expression : and_expression | exclusive_or_expression TOK_xor and_expression '''
...
def p_and_expression(p):
   ''' and_expression : equality_expression | and_expression TOK_and_bitwise equality_expression '''
...
def p_variable_argument_body(p):
   '''variable_declaration_body : identifiers type_opt TOK_assignment  variable_declaration_initializer 
...
def p_identifiers(p):
   ''' identifiers : identifiers TOK_comma TOK_identifier | TOK_identifier'''
...
def p_variable_list(p):
   ''' variable_list : variable_dec | variable_list TOK_comma variable_dec'''
...
def p_array_initializer(p):
   ''' array_initializer : KW_new KW_array TOK_lsqb type TOK_rsqb TOK_paraleft conditional_or_expression TOK_pararight
                                    | KW_array TOK_paraleft argument_list_opt TOK_pararight
                                    | KW_array TOK_lsqb type TOK_rsqb TOK_paraleft argument_list_opt TOK_pararight
...
def p_multidimensional_array_initializer(p):
   ''' multidimensional_array_initializer : KW_array KW_dot KW_ofdim TOK_lsqb type TOK_rsqb TOK_paraleft argument_list TOK_pararight'''
...
def p_class_initializer(p):
   ''' class_initializer : KW_new name TOK_paraleft argument_list_opt TOK_pararight '''
...
def p_for_statement(p):
   '''for_statement : KW_for TOK_paraleft for_logic TOK_pararight statement | KW_for TOK_lcurly for_logic TOK_rcurly statement'''
...
def p_fun_def(p):
   '''fun_def : fun_sig type_opt TOK_assignment 
                            | fun_sig type_opt'''
...
def p_fun_params(p):
   '''fun_params : fun_param | fun_params TOK_comma fun_param'''
